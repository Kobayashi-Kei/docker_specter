...
from inc.SpecterAttnPreds import SpecterAttnPreds
...
class Specter(SpecterAttnPreds):
...
def main():
    try:
        ...
        for epoch in range(args.num_epochs):
            train(model, tokenizer, train_loader, optimizer, scheduler, args.device, epoch, embedding, is_preds=True)
            if args.data_name != 'scincl':
                val_loss = validate(model, val_loader, args.device)
        ...
        # 評価
        embedding_axcell(model, tokenizer, args.version, args.device, is_preds=True)
        ...
        eval_log_CSFCube(model, tokenizer, args.device, args.version, is_preds=True)
